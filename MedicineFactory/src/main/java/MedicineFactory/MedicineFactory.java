...
public class MedicineFactory {
    ...
    public void run() {
        ...
        IOManager.getInstance().print(
                "[1 - 订单系统]\t[2 - 15个设计模式测试]",
                "[1 -訂單系統]\t[2 - 15個設計模式測試]",
                "[1 - Order System]\t[2 - 15 Design Patterns Test]"
        );

        int selectedSystem = -1;
        while ((selectedSystem = this.inputInteger()) == -1 || selectedSystem < 1 || selectedSystem > 2) {
...
//                        employee.stealMoney();
    ...
    public void designPatternTest(){
        while (true) {
            ...
            IOManager.getInstance().print(
                    "[6 - 迭代器模式]\t[7 - 多例模式]\t[8 - 适配器模式]\t[9 - 桥接模式]\t[10 - 组合模式]",
                    "[6 - 迭代器模式]\t[7 - 多例模式]\t[8 - 轉接器模式]\t[9 - 橋接模式]\t[10 - 組合模式]",
                    "[6 - Converter]\t[7 - Multiton]\t[8 - Adapter]\t[9 - Bridge]\t[10 - Composite]"
            );

            IOManager.getInstance().print(
                    "[11 - 中介者模式]\t[12 - 备忘录模式]\t[13 - 观察者模式]\t[14 - 状态模式]\t[15 - 策略模式]",
                    "[11 - 仲介者模式]\t[12 - 備忘錄模式]\t[13 - 觀察者模式]\t[14 - 狀態模式]\t[15 - 策略模式]",
                    "[11 - Mediator]\t[12 - Memento]\t[13 - Observer]\t[14 - State]\t[15 - Strategy]"
            );
